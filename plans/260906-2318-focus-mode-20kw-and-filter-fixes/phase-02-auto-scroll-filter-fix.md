...
## Requirements
### Functional
- [x] Update `NearbyUiHelper.shouldScrollToTop` to accept both `RefreshTriggerType.USER_REFRESH` and `RefreshTriggerType.FILTER_CHANGE`.
- [x] Update `NearbyViewModel.triggerFilterPipeline` or its caller methods (`toggleAcFilter`, `enterDcMode`, `exitDcMode`, `selectDcTier`, `applyCustomFilter`, `saveAndApplyCustomFilter`, `clearFilters`, `toggleWattage`) to pass `triggerType = RefreshTriggerType.FILTER_CHANGE`.
- [x] Ensure `lastRefreshTimestamp` is generated and assigned on `FILTER_CHANGE` as well as user refresh in `NearbyViewModel.executeFilterAndRoutingPipeline`.
- [x] Preserve reading position on background passive updates (`PASSIVE_BACKGROUND`).

### Non-Functional
- [x] Debounced and flicker-free animation in Compose `listState.animateScrollToItem(0)`.
- [x] No race conditions when list is empty (`itemCount == 0`).

## Implementation Steps
1. [x] Modify `app/src/main/java/com/evcs/favorites/ui/components/NearbyUiHelper.kt`:
...
3. [x] Create single test file: `app/src/test/java/com/evcs/favorites/ui/NearbyAutoScrollFilterFixTest.kt`:
...
## Test Criteria
- [x] Run `./gradlew testDebugUnitTest --tests "com.evcs.favorites.ui.NearbyAutoScrollFilterFixTest"`
- [x] 100% tests pass.
