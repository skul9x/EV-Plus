...
- [x] Update `FavoritesViewModel.kt`:
  - In `enrichTopFullStationsWithForecast` (rename or update to `enrichTopStationsWithForecast`):
...
- [x] Ensure pull-to-refresh (`forceRefresh = true`) triggers fresh enrichment for the top 5 stations in both screens.

### Non-Functional
- [x] Performance: Maximum 5 concurrent/throttled network requests via existing repository `Semaphore(3)`.
- [x] Clean Degrade: If network fails or station has no active sessions, `station.forecast` remains null with zero UI flickering or crashes.

## Implementation Steps
1. [x] Update `NearbyViewModel.kt` target station selection to `top10DisplayStations.take(5)`.
2. [x] Update `FavoritesViewModel.kt` target station selection to `favoriteStations.take(5)`.
3. [x] Create single test file `Top5UnconditionalForecastViewModelTest.kt`.
4. [x] Run unit test to verify that top 5 stations in both ViewModels are queried regardless of port availability.
...
- [x] Verifies `NearbyViewModel` queries Top 5 stations when stations have available ports (`totalAvailablePlugs > 0`).
- [x] Verifies `NearbyViewModel` attaches forecast to stations when repository returns valid forecast data.
- [x] Verifies `NearbyViewModel` leaves `forecast = null` when repository returns null (skips without dummy data).
- [x] Verifies `FavoritesViewModel` queries Top 5 favorite stations unconditionally.
- [x] Verifies station sort order, driving metrics, and favorite status remain preserved after forecast enrichment.
