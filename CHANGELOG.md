...
## [2026-09-06] - VinFast CAPP Direct API Integration & Dual-Tier Fallback Architecture (Planning Phase)

### Added
- **Architecture Brief & Analysis**:
  - Reverse-engineered VinFast Connected Car (CAPP) Android app APK (`output_vinfast`) to extract live charging station endpoints and client identity headers (`X-APP-VERSION: 2.25.7`, `X-SERVICE-NAME: CAPP`, `X-Device-Platform: android`).
  - Probed live VinFast and EVCS APIs to verify endpoint behaviors, query vs body parameters, and 401 authentication dynamics.
- **Dual-Tier Fallback Specification**:
  - Formulated 4-phase implementation plan with Dual-Tier architecture: Tier 1 (VinFast Direct API) -> Tier 2 (EVCS HMAC-SHA256 Fallback).
  - Enforced strict EV car-only port filter (stripping motorbike plugs `<= 7kW`, retaining 11kW AC and 30kW - 360kW DC).
  - Verified direct S3 CDN image decoding (`https://cpo-prod-s3.vinfastauto.com/`) to bypass 3rd party Cloudflare 403 blocks.
  - Preserved 100% Cloud Firestore user favorites schema compatibility (`users/{uid}/userdata/favorites`).
